...
//     wxBrush semiWhite(image1);
		...
		if (textY != mpLEGEND_MARGIN) { // Don't draw any thing if there are no visible layers
			...
			for (unsigned int p2 = 0; p2 < w.CountAllLayers(); p2++) {
				 ...
				 * Inseri a terceira condição (ly->GetName() != "") pra ele não criar espaços nas legendas quando um
				 * layer sem nome for inserido. Do jeito que estava, se 10 layers forem inseridos e apenas os 2 últimos
				 * tiverem nome, a legenda fica muito estranha. Isso acontecia na adaptação do gráfico polar.
				 */
				if ((ly->GetLayerType() == mpLAYER_PLOT) && (ly->IsVisible()) && (ly->GetName() != "")) {
					label = ly->GetName();
					lpen = ly->GetPen();
					dc.GetTextExtent(label, &tmpX, &tmpY);
					dc.SetPen(lpen);
...
void mpScaleY::Plot(wxDC & dc, mpWindow & w)
{
	...
	//     }
		...
		// Adicionado por Leandro
//		if (n < 0.01) {
//			s = Converter::DoubleToScientificNotationString(n, 2);
//		} else {
//			s = Converter::DoubleToFormattedString(n, 2);
//		}
		// FIM Adicionado por Leandro
...
BEGIN_EVENT_TABLE(mpWindow, wxWindow)
    ...
    EVT_MENU( wxID_SAVE,      mpWindow::OnSavePlot) /* ADICIONADO POR LEANDRO */
    ...
    : wxWindow( parent, id, pos, size, flag, wxT("mathplot") )
{
    ...
    // ADICIONADO POR LEANDRO
    m_popmenu.Append( wxID_SAVE,   _("Save plot..."),    _("Save plot."));
    // FIM ADICIONADO POR LEANDRO
...
{
	wxString wildcards = "BMP files (*.bmp)|*.bmp";
	/**
	 * TODO: TESTAR NO WINDOWS 7.
	 * POR ENQUANTO DEIXAR SALVANDO APENAS BMP
	wildcards.Append("|GIF files (*.gif)|*.gif").
			Append("|JPEG files (*.jpg)|*.jpg").
			Append("|PNG files (*.png)|*.png").
			Append("|TIF files (*.gif)|*.tif");
	 */
	wxFileDialog dialog(this, "Save plot", "", "", wildcards, wxFD_SAVE);
	...
	if (dialog.ShowModal() == wxID_OK) {
		 ...
		 */
		bool saveOK = SaveScreenshot(fileName, type, this->GetSize(), true);
		if (saveOK) {
			wxString msg = fileName.Append(" saved.");
			wxMessageDialog msgDlg(NULL, msg, "File saved", wxICON_INFORMATION|wxOK);
			msgDlg.ShowModal();
		} else {
			wxString msg = "Error saving "; msg.Append(fileName).Append(".");
			wxMessageDialog msgDlg(NULL, msg, "Error", wxICON_ERROR|wxOK);
			msgDlg.ShowModal();
		}
	}
...
//        - Wheel + CTRL: Zoom in/out"),_("wxMathPlot help"),wxOK,this);
    ...
    wxMessageBox(_("Supported Mouse commands:\n \
        ...
        - Wheel: Vertical scroll\n \
        - Wheel + SHIFT: Horizontal scroll\n \
        - Wheel + CTRL: Zoom in/out"),_("EM Studio"),wxOK,this);
    // FIM Adicionado por Leandro
}
 ...
 * Adicionado por Leandro
 */
bool mpWindow::AddToggleVisibleLayer( mpLayer* layer, bool refreshDisplay) {
	if (layer != NULL) {
		m_layers.push_back( layer );
		if (refreshDisplay) UpdateAll();

		if (firstToggledVisibleLayer) {
			m_popmenu.AppendSeparator();
			firstToggledVisibleLayer = !firstToggledVisibleLayer;
		}
		wxMenuItem* menuItem = m_popmenu.AppendCheckItem(wxID_ANY, _(layer->GetName()), _(layer->GetName()));
		toggledVisibleLayersNames.push_back(std::pair<wxString, int>(layer->GetName(), menuItem->GetId()));
		menuItem->Check(layer->IsVisible());
		Connect(menuItem->GetId(), wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(mpWindow::OnShowHideLayers));
 ...
 * Adicionado por Leandro
 */
void mpWindow::OnShowHideLayers(wxCommandEvent& event) {
//	cout << "OnShowHideLayers" << endl;
	wxMenuItem* menuItem = m_popmenu.FindChildItem(event.GetId());
	wxString labelName = menuItem->GetItemLabelText();

	for (size_t i = 0; i < m_layers.size(); i++) {
		if (m_layers[i]->GetName() == labelName ) {
			m_layers[i]->SetVisible(!(m_layers[i]->IsVisible()) );
			menuItem->Check(m_layers[i]->IsVisible());
			UpdateAll();
			break;
		}
	}
...
bool mpWindow::DelLayer(
    ...
    bool        refreshDisplay )
{
    /** Inserido por Leandro */
	std::vector<std::pair<wxString, int> >::iterator iBegin = toggledVisibleLayersNames.begin();
	std::vector<std::pair<wxString, int> >::iterator iEnd = toggledVisibleLayersNames.end();
	for (std::vector<std::pair<wxString, int> >::iterator i = iBegin; i < iEnd; i++) {
    	if (layer->GetName() == i->first) {
    		m_popmenu.Delete(i->second);
    		toggledVisibleLayersNames.erase(i);
    		break;
    	}

	}
    /** FIM Inserido por Leandro */
...
{
    /** Inserido por Leandro */
    for (size_t i = 0; i < toggledVisibleLayersNames.size(); i++) {
   		m_popmenu.Delete(toggledVisibleLayersNames[i].second);
    }
    toggledVisibleLayersNames = std::vector<std::pair<wxString, int> >();
    /** FIM Inserido por Leandro */
...
//        cout << "\t\t Após f->GetName() = " << f->GetName() << ", m_maxY = " << m_maxY << endl;
        //node = node->GetNext();
    }
    /* Alterado por Leandro. Cria um espaço entre o mínimo e o máximo (ver comentário em  mpFXYVector::SetData() ) */
    double diff = m_maxY-m_minY;
    if (diff == 0) {diff = m_maxY; }
	m_minY-=(0.1*diff);
	m_maxY+=(0.1*diff);
	/* Fim da alteração - Leandro */
#ifdef MATHPLOT_DO_LOGGING
...
bool mpWindow::SaveScreenshot(const wxString& filename, int type, wxSize imageSize, bool fit)
{
    ...
    return screenImage.SaveFile(output, type);
    // return screenImage.SaveFile(filename, type); COMENTADO POR LEANDRO -> ESSE ERA O CÓDIGO ORIGINAL, DEPRECIADO
}
...
void mpFXYVector::SetData( const std::vector<double> &xs,const std::vector<double> &ys)
{
    ...
    if (xs.size()>0)
    {
         ...
         * Alterado por Leandro.
         *
         * Se deixar esse código, sempre vai ficar um espaço de 0.5 entre o máximo e o mínimo da curva.
         * Para grandes valores isso não tem problema, mas para gráficos com pequenos valores a curva fica muito achatada.
         * Se passar só o valor correto, a biblioteca não funciona quando o gráfico é praticamente uma linha horizontal.
         * Por isso, comentei essas linhas e coloquei no método mpWindow::UpdateBBox() um código para dar um espaço entre o máximo e o mínimo da curva.
         */
        /*
         * m_minX-=0.5f;
         * m_minY-=0.5f;
