...
public:
    ...
    bool Load(wxString stream);
    bool IsOk() const { return m_loaded; }
//    void Render(bool fill) const;

    void SetEntityList(DXFEntityList& list);
    void SetLayerList(DXFLayerList& list);
    DXFEntityList& GetDXFEntityList();
    DXFLayerList& GetLayerList();
    vector<Facet> GetFacets() { return facets; }

    void Transform(bool normalize, double rotX, double rotY, double rotZ, double transX, double transY, double transZ, double scale);

    //Operador atribuição
    DXFParser& operator=(DXFParser& other);
private:
