...
*/
void nec_context::xq_card(int itmp1, InformSimulationStatusSmartPointer statusSP, int initialPercentage, int totalPercentage)
{
	...
	simulate(true, statusSP, initialPercentage, totalPercentage);
}
...
void nec_context::rp_card(int calc_mode, int n_theta, int n_phi,
	...
	InformSimulationStatusSmartPointer statusSP, int initialPercentage, int totalPercentage)
{
...
*/
void nec_context::simulate(bool far_field_flag, InformSimulationStatusSmartPointer statusSP, int initialPercentage, int totalPercentage)
{
	...
	*/
	...
	{
		if (statusSP.get() != 0) {
			double total = (1.0*mhz)/nfrq;
			statusSP->Update((int)(initialPercentage + total*totalPercentage));
		}
		switch( igox )
		{
	...
	while( (++mhz <= nfrq) && (statusSP.get() == 0 || !statusSP->SimulationWasCanceled()));

	if (statusSP.get() != 0 && !statusSP->SimulationWasCanceled()) {
		statusSP->Update((int)(initialPercentage + totalPercentage));
	}
