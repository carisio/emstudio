...
// now proceed and complete the geometry setup...
	...
	/*
		...
		only for closed perfectly conducting surfaces. ï¿½You start with a
		single patch and connect a wire to its center so that it splits into
		four patches. ï¿½You would need to use the SM command to make a surface
		with more patches, then use other SM or MV commands to form the other
		five faces of a closed box. ï¿½People often do
		use the NEC patch model for non-closed surfaces, and it is completely
		invalid. ï¿½It would be very useful to have a check for non-closed
		patch surfaces in NEC-2 or in GUIs written for it , but determining
		whether a bunch of patches forms a closed surface is not easy. ï¿½If
...
// INICIO COMENTADO LEANDRO
// ESSE CÓDIGO FOI COMENTADO PORQUE NÃO ESTAVA RODANDO OS EXEMPLOS
// TODA VEZ, ERA IDENTIFICADO QUE UM FIO CONECTAVA OUTRO, MESMO A CONEXÃO DELES SENDO FEITA PELOS CENTROS
// ESSE CÓDIGO FOI INSERIDO NO GITHUB, MAS NÃO ESTÁ NA VERSÃO DE 2004
...
// Adicionado por Leandro -> Essa versão força nrpt = 0.
// O objetivo dela é permitir usar o cartão GX em apenas um conjunto limitado de segmentos
// A versão original aplica o método move em todos os seguimentos que estão armazenados após tag = its
// Nessa versão, aplica o método move em nseg que estão armazenados após tag = its
void c_geometry::move( nec_float rox, nec_float roy, nec_float roz, nec_float xs,
    nec_float ys, nec_float zs, int its, int nrpt, int itgi, int nseg )
{
	nrpt=0; // Adicionado por Leandro
  int nrp, ix, i1, k, ir, i, ii;
  nec_float sps, cps, sth, cth, sph, cph, xx, xy;
  nec_float xz, yx, yy, yz, zx, zy, zz, xi, yi, zi;
  ...
  if ( n > 0)
  {
	...
	k++;
      ...
      i1= n+1;
//      n= k; Comentado por Leandro
      // Como nessa versão o nrpt é forçado para 0, alterei isso daqui
      // Se isso não for feito, dá pau na hora de chamar connect_segments, pq na versão original o loop sempre vai até n, então n assume o valor de k,
      // que é igual a n após a primeira saída do loop
