...
CircularTube::CircularTube() : ElementsWithFacets(ElementTypeSmartPointer(new Object3DType())) {
...
ElementGUISmartPointer CircularTube::NewElementGUI() {
	return ElementGUISmartPointer(new CircularTube());
}

wxString CircularTube::GetRegisteredName() {
	return "CircularTube";
}

wxIcon CircularTube::GetIcon() {
	return UtilResource::GetIcon("CircularTube");
}

ElementDTOSmartPointer CircularTube::NewElementDTO() {
	CircularTubeDTO* result = new CircularTubeDTO;

	return ElementDTOSmartPointer(result);
}
bool CircularTube::FillApproximateDimensions(double& xmin, double& ymin, double& zmin, double& xmax, double& ymax, double& zmax) {
	if (!visible.IsValue())
		return false;
...
void CircularTube::Draw(OpenGLStateMachine* opengl) {
	if (!visible.IsValue())
	  return;
...
bool CircularTube::IsAllPropertiesTogetherValid() {
 ...
 */
std::vector<Facet> CircularTube::BuildFacets() {
...
void CircularTube::DoMove(Point3DME p) {
